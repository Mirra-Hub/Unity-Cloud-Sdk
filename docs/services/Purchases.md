...
(раздел «Интеграции»: Stripe, ЮKassa, VK Игры, Google Play) и адресуется её **ключом**. Ключ выдаёт облако
при создании интеграции: ключ сервиса (`stripe`), для следующих интеграций того же сервиса — `stripe-2`,
`stripe-3`. Ключи и секреты провайдера живут в интеграции, а не в ветке: в SDK их нет.

У одного товара может быть несколько цен — по одной на интеграцию (например, `stripe` в EUR и
