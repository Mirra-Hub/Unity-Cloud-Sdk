...
- **Platform** — платформа сборки: её ключ пишется в `Configuration.PlatformKey` и уходит с каждым входом и запросом
  аналитики. В пункте списка — имя, ключ и типы платформы (`Web | PC | Mobile | Console`, задаются в консоли).
  Сохранённый ключ, если он есть в проекте, не меняется; иначе выбирается первая включённая платформа под тип
  активного билд-таргета, за ней — первая включённая, за ней — первая. Список — client-роут платформ через sa-gateway;
  сервисному аккаунту нужно право `platforms.viewer`. Проект без платформ — предупреждение: вход в нём отклоняется,
  пока платформу не заведут в консоли. На 5xx окно пишет, что делать, а статус, код ошибки и URL уходят в консоль
- **Проверка билд-таргета** — красное сообщение под Platform (с кнопкой Build Settings…), если тип активного таргета
  не входит в типы выбранной платформы (`BuildTargetPlatformType`): Standalone → PC, WebGL → Web, Android/iOS →
  Mobile, PlayStation/Xbox/Switch → Console. tvOS, visionOS, UWP, embedded, Dedicated Server и платформа без типов не
  проверяются. Таргет читается на каждой перерисовке, `ActiveBuildTargetListener` перерисовывает окно при его смене
